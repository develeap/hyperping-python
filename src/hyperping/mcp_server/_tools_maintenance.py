...
def register_maintenance_tools(
    ...
    if isinstance(client, AsyncHyperpingClient):
        ...
        @mcp.tool()
        async def get_maintenance(maintenance_id: str) -> dict[str, Any]:
            """Get a single maintenance window by UUID."""
            return (await client.get_maintenance(maintenance_id)).model_dump()
        ...
        async def create_maintenance(
            ...
            return (await client.create_maintenance(MaintenanceCreate(**fields))).model_dump()
        ...
        async def update_maintenance(
            ...
            return (
                await client.update_maintenance(maintenance_id, MaintenanceUpdate(**fields))
            ).model_dump()

        @mcp.tool()
        async def delete_maintenance(maintenance_id: str) -> dict[str, Any]:
            """This will permanently delete a maintenance window."""
            await client.delete_maintenance(maintenance_id)
            return {"success": True}

        @mcp.tool()
        async def get_active_maintenance() -> list[dict[str, Any]]:
            """List currently active maintenance windows."""
            return [m.model_dump() for m in await client.get_active_maintenance()]

        @mcp.tool()
        async def is_monitor_in_maintenance(monitor_uuid: str) -> dict[str, Any]:
            """Check whether a monitor is currently inside an active maintenance window."""
            result = await client.is_monitor_in_maintenance(monitor_uuid)
